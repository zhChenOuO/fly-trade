...
- G1（合成基準）：規格見 `research/G1_SPEC_v2_draft.md`（取代 v1 draft）。**未結案，結論待定**（見 `research/RESULTS_G1_v2.md` 2026-09-22 更正紀錄）：原 VALID_GATE_FAIL 判定已撤回，Stage B 的 d2 診斷 target 存在 Blocker 級設計缺陷（在此讀出狀態下數學上不可預測，與連接體/拓樸/讀出能力無關），Claude 已獨立驗證（R1-6 與 DN 索引完全不重疊、圖中無任一條直接突觸）。不進 Stage C/D，但理由是「尚無有效能力證據」，不是「已證明失敗」。
- G2（僅 G1 通過）：BTC 已實現波動度，對照 HAR，新 holdout。**維持不解鎖（G1 未有結論）。**
- G3（僅 G2 通過）：微結構資料；不由「某論文有訊號」自動解鎖。**維持不解鎖。**

## 4. 判定
- 本 pipeline（5m OHLCV→圖像）不值得再作確認性測試。這不推出「市場方向不可預測」的普遍結論。
- 「先驗約 2%」為 Codex 主觀估計，非資料估計。
- G1 結果（`RESULTS_G1_v2.md`）：Stage B 的非線性能力診斷因設計缺陷無效，未產生任何有效結論；線性延遲診斷（R²=0.578）是可信觀測，證明監督式學習（Ridge）本身有效、DN 讀出保有線性記憶，但尚不構成拓樸優於隨機的證據。波動度與微結構路徑（G2/G3）因此維持不解鎖，待 G1 重新設計後才能決定。
