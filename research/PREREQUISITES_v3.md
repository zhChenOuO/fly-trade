...
- G0（工具）：充分混合 scramble、權重洗牌、無偏隨機圖、圖 provenance；MDE 標籤注入（母體係數設計）。不重評市場舊 Val；G0b（舊市場特徵重跑新對照）不做。**已完成。**
- G1（合成基準）：規格見 `research/G1_SPEC_v2_draft.md`（取代 v1 draft）。**已結案：STAGE_C_NO_GO**（見 `research/RESULTS_G1_v2.md` 2026-09-24 最終判定）。主測完整執行：real FlyWire 在 NARMA10 的 NMSE（0.512）比全部 20 個 scramble_mixed 對照（0.403–0.422）都差，Δ=−0.253，95% CI [−0.275,−0.234] 完全為負。依預先議定停損規則，不重跑、不換設定。connectome-as-reservoir 一般計算能力研究線結束。
- G2（僅 G1 通過）：BTC 已實現波動度，對照 HAR，新 holdout。**維持不解鎖（G1 已結案為 NO-GO，非連接體加值路線的前提不成立）。**
- G3（僅 G2 通過）：微結構資料；不由「某論文有訊號」自動解鎖。**維持不解鎖。**

## 4. 判定
- 本 pipeline（5m OHLCV→圖像）不值得再作確認性測試。這不推出「市場方向不可預測」的普遍結論。
- 「先驗約 2%」為 Codex 主觀估計，非資料估計。
- G1 最終結果（`RESULTS_G1_v2.md`）：real FlyWire 拓樸在 NARMA10 合成基準上，不只沒有優於隨機重接對照，反而系統性地更差（輸給全部 20 個對照）。這是有資訊量的負結果，不是「證據不足」。「connectome-enhanced」市場路線（G2/G3 的加值版本）因此維持不解鎖；若要做市場研究，應走獨立的非 connectome 路線（見 §3 排序），不再依賴果蠅拓樸有計算優勢這個前提。
