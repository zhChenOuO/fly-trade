...
- Analytical spectral Ridge solver matching np.linalg.lstsq at alpha=0.
- Saturation, initial-state forgetting, and linear memory capacity gates.
...
def generate_g1_splits() -> dict[str, list[dict]]:
    """Generate all frozen NARMA10 sequences for Train, Val, and Test splits."""
    train_seqs = [
        generate_narma10_sequence(seed=s, valid_length=TRAIN_SEQUENCE_LENGTH, washout=WASHOUT_STEPS)
        for s in TRAIN_SEEDS
    ]
    val_seqs = [
        generate_narma10_sequence(seed=s, valid_length=VAL_SEQUENCE_LENGTH, washout=WASHOUT_STEPS)
        for s in VAL_SEEDS
    ]
    test_seqs = [
        generate_narma10_sequence(seed=s, valid_length=TEST_SEQUENCE_LENGTH, washout=WASHOUT_STEPS)
        for s in TEST_SEEDS
    ]
...
# ==============================================================================
# Spectral Ridge Readout with Sequence-Group 5-Fold CV (SPEC §3)
# ==============================================================================

class G1RidgeReadout:
    """Ridge Readout with unpenalized intercept and spectral decomposition caching.
    ...
    Spectral Solver:
        G = X_c^T X_c = V diag(lambda) V^T
        B(alpha) = V diag( 1 / (lambda_i + n * alpha) ) V^T X_c^T Y_c
        At alpha=0: pseudo-inverse with cutoff lambda_i < 1e-12 (matches np.linalg.lstsq).
    """
    ...
    def _fit_spectral(
        ...
        """Solve for B and intercept given precomputed Gram eigendecomposition."""
        n = len(X_c)
        if alpha > 0.0:
            denom = eigvals + n * alpha
            B = V @ (z / denom)
        else:
            # Pseudo-inverse with 1e-12 cutoff
            inv_eig = np.zeros_like(eigvals)
            pos_mask = eigvals > 1e-12
    ...
    def fit_sequence_group_cv(
        ...
        Each fold holds out 2 sequences as validation, fits on the other 8,
        evaluates NMSE across the alpha grid, and selects the best alpha.
        """
        n_seqs = len(sequence_features)
        assert n_seqs == 10, f"Expected exactly 10 Train sequences for group CV, got {n_seqs}"
        seq_indices = np.arange(n_seqs)
        ...
        for f in range(n_folds):
            ...
            # Eigendecomposition of Gram matrix (d x d)
            G = Xc_tr.T @ Xc_tr
            ...
            for a in self.alphas:
                B_a, b0_a = self._fit_spectral(Xc_tr, yc_tr, mu_x_tr, mu_y_tr, eigvals, V, z, a)
        ...
        G_all = Xc_all.T @ Xc_all
        eig_all, V_all = np.linalg.eigh(G_all)
        order_all = np.argsort(eig_all)[::-1]
        eig_all = np.maximum(0.0, eig_all[order_all])
        V_all = V_all[:, order_all]
        z_all = V_all.T @ (Xc_all.T @ yc_all)

        self.weights, self.intercept = self._fit_spectral(
            Xc_all, yc_all, mu_x_all, mu_y_all, eig_all, V_all, z_all, self.best_alpha
        )
        ...
        self.cv_results = {
            ...
            "alpha_at_boundary": bool(self.best_alpha == self.alphas[0] or self.best_alpha == self.alphas[-1]),
...
def check_saturation_gate(
    ...
    """Check saturation gate (a): post-washout non-sensory |x| > 0.9 proportion < 5%."""
    # states shape: (B, T, N) or (T, N)
    if states.ndim == 2:
        states = states[None, ...]
    B, T, N = states.shape

    sensory_set = set(int(i) for i in sensory_idx)
    non_sensory_mask = np.array([i not in sensory_set for i in range(N)])

    non_sensory_states = states[:, :, non_sensory_mask]
    sat_count = int(np.sum(np.abs(non_sensory_states) > threshold))
    total_non_sensory_points = int(non_sensory_states.size)

    sat_ratio = float(sat_count / total_non_sensory_points) if total_non_sensory_points > 0 else 0.0
...
def check_forgetting_gate(
    sim_fn: Callable[[np.ndarray, np.ndarray | None], tuple[np.ndarray, np.ndarray]],
    ...
    for s_idx, u_full in enumerate(train_u_sequences):
        ...
        for p_idx in range(n_pairs_per_seq):
            ...
            # State b: x0 ~ U[-0.1, 0.1]
            x0_b = rng.uniform(-0.1, 0.1, size=n_neurons).astype(np.float32)

            _, final_a = sim_fn(u_500, x0_a)
            _, final_b = sim_fn(u_500, x0_b)

            # Compute relative RMS difference at t=500
...
def compute_memory_capacity(
    ...
    """Compute Linear Memory Capacity: MC = sum_{k=1..50} max(R^2(u[t-k]), 0)."""
    # states shape: (T, N), u shape: (T,)
    T = len(u)
...
def build_narma10_oracle_features(u: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    At step t:
        terms: y[t], y[t] * sum_{k=0..9} y[t-k], u[t] * u[t-9]
    Target is y[t+1].
    """
    T = len(u)
...
def evaluate_oracle_positive_control(splits: dict[str, list[dict]]) -> dict:
    """Verify that oracle teacher features achieve NMSE < 1e-6 across all splits."""
    results = {}
...
def evaluate_negative_control(
    ...
    # Negative control must not show spurious improvement (NMSE must be >= 1.0 within numerical noise)
