...
class TorchReservoir:
    ...
    def max_batch_size(self, steps: int = 32, n_sensory: int | None = None) -> int:
    ...
    def simulate_batch(
        ...
        if batch_size == 0:
            ...
            if return_final_state:
                return empty_motor, np.empty((0, self.n_neurons), dtype=np.float32)
            return empty_motor

        gains = self._parameter_vector(gain, batch_size, "gain")
        leaks = self._parameter_vector(leak, batch_size, "leak")
        chunk_size = self.max_batch_size(steps=steps, n_sensory=n_sensory)
